...
def getBackTestList(price_df: pd.DataFrame, moment_generating_func: Callable[[pd.DataFrame], pd.DataFrame], max_window_years: int = 5, getXYgXgY = TF_RA, nr_moments: int = 4, useConvex: bool = False):
...
if __name__ == "__main__":
    ...
    for (useConvex ,cur_nr_moments, cur_mom_gen_func) in tqdm(product([False, True], range(4,5), [MVSK, LMoments])):
        testlist = getBackTestList(data_prices.sort_index(), moment_generating_func=cur_mom_gen_func, max_window_years=5, getXYgXgY=TF_RA, nr_moments=cur_nr_moments, useConvex=useConvex)
        res = bt.run(*testlist)
